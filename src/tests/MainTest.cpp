...
// g++ [MainTest.cpp] [DEPENDENCY FILES ON TESTABLE CLASS] [TESTABLE CLASS]
// [TEST CLASS] `pkg-config --cflags --libs opencv` [or other flags] example:
// g++ src/tests/MainTest.cpp src/ViewChanger.cpp src/ContourFinder.cpp
// src/tests/TestContourFinder.cpp `pkg-config --cflags --libs opencv`
